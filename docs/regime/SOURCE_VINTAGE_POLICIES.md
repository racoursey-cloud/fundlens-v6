...
- **Adoption fetch evidence (production): PENDING EXECUTION.** The adoption run emails its own fetch timestamp, first data row, CRSP build note, and both SHA-256s (zip as fetched; extracted CSV); those values land here by amendment, reconciled against the prints above. Until that amendment, this section's evidence is the S4 print.
- **License: PENDING PRIMARY READ — adoption is not complete until this lands.** The library's terms live on its own pages, unreachable from this build container (403, register below). The quote lands here by amendment via the Fabio-side channel (the S4 print precedent) or the operator's browser, before the runner uses the rail. Charter §4.2·7 requires the license read at adoption; §4.2·9 stands — if the terms are not free for this use, that is a STOP, not a purchase.
...
- **Earliest honestly-replayable date:** 1926-07-01 under the frozen-at-adoption policy (primary print, July 10, 2026; production ingest confirms and its print is amended in here).
