...
- **ERRATA (July 7, 2026, found by the first backfill execute):** an `alfred` label in the registry is a claim ALFRED may not honor — **CFNAI is not in ALFRED at all** (FRED 400s any realtime request for it: "The series does not exist in ALFRED"). The backfill now discovers each series' true archive at runtime: absent archives fall back to today-dated snapshot storage and are reported as findings in the completion email; present archives clamp the replay grid to their first vintage date. CFNAI's registry `vintage_policy` should be corrected to `snapshot_custom` (Robert's Table Editor cell edit, pending); its as-published replay begins at our first snapshot. Consequence for the race, flagged for the race assignment alongside the OFR item (§4): pre-snapshot CFNAI history is revised-basis only.
